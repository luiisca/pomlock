...
class TimerEngine:
    ...
    def skip(self, force: bool = False) -> None:
        ...
        logger.debug(f"tick: state={self.state} elapsed_s={self.elapsed_s} duration_s={self.duration_s}")
    def tick(self) -> None:
    ...
    def _capture_elapsed(self) -> None:
        logger.debug(f"_capture_elapsed: state={self.state} last_tick={self._last_tick_time} now={time.time()} delta={now - self._last_tick_time if self.state == TimerState.RUNNING else 0}")
