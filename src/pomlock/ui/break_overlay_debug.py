...
def run_standalone_overlay(
    ...
    setup_tcl_env()
    print(f"[DEBUG] After setup_tcl_env: {time.time() - _start_time:.3f} seconds", flush=True)
    ...
    if is_hyprland:
        ...
        root.update()
        print(f"[DEBUG] After first root.update: {time.time() - _start_time:.3f} seconds", flush=True)
        ...
        root.update()
        print(f"[DEBUG] After second root.update: {time.time() - _start_time:.3f} seconds", flush=True)
        ...
        for c in canvases:
            ...
            draw_overlay_frame(c, break_title, current_time[0], accent_color)
        root.update()
        print(f"[DEBUG] After root.update post-draw: {time.time() - _start_time:.3f} seconds", flush=True)
    else:
    ...
    print(f"[DEBUG] Before root.mainloop: {time.time() - _start_time:.3f} seconds", flush=True)
    try:
        root.mainloop()
